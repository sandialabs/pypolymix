...
def prepare_error_visualization_dataframe(df):
    """
    Convert the compact error CSV format into the input/reference/predicted
    column format expected by make_gui_error_visualization_files().
    """
    ...
    for col in input_columns:
        ...
        df[f"input_{col}"] = df[col]

    # These suffixes determine the GUI output names:
    #   reference_devm_dt  -> output_devm_dt
    #   reference_drhom_dt -> output_drhom_dt
    #   reference_drhoi_dt -> output_drhoi_dt
    error_output_map = {
        "raw_nrmse_evm": "devm_dt",
        "raw_nrmse_rhom": "drhom_dt",
        "raw_nrmse_rhoi": "drhoi_dt",
    }

    for error_col, suffix in error_output_map.items():
        if error_col not in df.columns:
            raise KeyError(f"Missing required output/error column: {error_col}")

        ref_col = f"reference_{suffix}"
        pred_col = f"predicted_{suffix}"

        # Dummy values chosen so that relative difference is the supplied error:
        #
        #   (predicted - reference) / reference
        #   = ((1 + error) - 1) / 1
        #   = error
        df[ref_col] = 1.0
        df[pred_col] = 1.0 + df[error_col].astype(float)
...
def make_gui_error_visualization_files(
    ...
    input_names = [column for column in df.columns if not column.startswith("raw")]
    reference_names = [column for column in df.columns if column.startswith("reference")]
    predicted_names = [column for column in df.columns if column.startswith("predicted")]
    ...
    if output_dir is None:
        output_dir = f"reference_results_{input_x}_{input_y}"
...
Outputs 3 files in the specified output folder, 
or in a new folder called reference_results<input1>_<input2> if no output folder is provided.
...
- SM.pickle: A supplementary file that the GUI requires to be in the same directory as train_output.pickle""",
    ...
    parser.add_argument(
        "csv_file",
        help="CSV file containing input and relative-error columns.",
    )

    parser.add_argument(
        "input_variable_1",
        help="First input variable to visualize. Use one of: temperature, vmJ2, flux.",
    )

    parser.add_argument(
        "input_variable_2",
        help="Second input variable to visualize. Use one of: temperature, vmJ2, flux.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If omitted, defaults to reference_results<input1>_<input2>"
        )
    )
    ...
    df = prepare_error_visualization_dataframe(df)
